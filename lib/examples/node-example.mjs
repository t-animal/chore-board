...
import { ChoresApi, getGcalApi, listCalendars } from "../dist/index.js";
...
async function main() {
  ...
  try {
    const gCalApi = await getGcalApi(
      {
    ...
    const calendars = await listCalendars(gCalApi);
    console.log("Calendars:");
    console.dir(calendars, { depth: 2 });

    const choresCalendar = calendars.find((c) => c.title === "Chores");
    if (!choresCalendar) {
      throw new Error('Calendar titled "Chores" not found; pass a calendar id instead.');
    }

    const choresApi = new ChoresApi(gCalApi, choresCalendar.id);
    const chores = await choresApi.getChores(10, 30, 50);
    console.log("Chores:");
    for(const choresEntry of chores) {
      console.log(`- ${choresEntry.title} (due @${choresEntry.dueDate.toLocaleString()})`);
      if(choresEntry.description) {
        console.log(`  Description: ${choresEntry.description.substring(0, 50)}${choresEntry.description.length > 50 ? "..." : ""}  `);
      }
    }
  } catch (err) {
...
async function authorizeOauthClient(clientId, clientSecret, existingCredentials) {
  ...
  if (existingCredentials) {
    oauth2Client.setCredentials(
      JSON.parse(Buffer.from(existingCredentials, "base64").toString("utf-8")),
    );
    return oauth2Client;
  }
...
async function promptForCode(oauth2Client) {
  ...
  const code = await rl.question(`Enter the authorization code from the page here: `);
  rl.close();
